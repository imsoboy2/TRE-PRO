...
def populate_hot_flow_rule(p):
    ...
    print rule
    cmd = 'echo \"%s\" | /home/p4/behavioral-model/targets/simple_switch/sswitch_CLI --thrift-port 9091' % rule
    os.system(cmd)
...
def main():
    ifaces = filter(lambda i: 'veth13' in i, os.listdir('/sys/class/net/'))
    iface = ifaces[0]
