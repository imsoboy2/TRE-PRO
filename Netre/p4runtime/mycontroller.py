...
def printRegister(p4info_helper, sw, register_name, index):
    ...
    :param index: the counter index (in our case, the tunnel ID)
    """
    for response in sw.ReadRegisters(p4info_helper.get_registers_id(register_name), index):
        for entity in response.entities:
...
def getCounterValue(p4info_helper, sw, counter_name, index):
    ...
    for response in sw.ReadCounters(p4info_helper.get_counters_id(counter_name), index):
        for entity in response.entities:
            counter = entity.counter_entry
            cnt = counter.data.packet_count 
            if cnt > prev_cnt:
                print "%dth hot flow" % cnt
                prev_cnt = cnt 
                printRegister(p4info_helper, sw, "MyIngress.hot_flow_value", 0)
...
def main(p4info_file_path1, bmv2_file_path1, p4info_file_path2, bmv2_file_path2):
    ...
    try:
        ...
        while True:
            sleep(2)
            # printCounter(p4info_helper, s1, "MyIngress.report_counter", 0)
            # getCounterValue(p4info_helper, s1, "MyIngress.report_counter", 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='../build/tre_ingress.p4.p4info.txt')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='../build/tre_ingress.json')
    ...
    main('../build/tre_ingress.p4.p4info.txt', '../build/tre_ingress.json', '../build/tre_egress.p4.p4info.txt', '../build/tre_egress.json')
