...
parser.add_argument('--dist', required=False, default='z99_dist', help='flow distribution')
...
def make_packet(srcip, payload):
  ether = Ether(dst=get_if_hwaddr('veth1'))
...
def main():
  ...
  for j in range(0, 1):        
    for i in range(len(set_of_ip)):
      idx = int(set_of_ip[i], 16) + (5000 / switchnum) * j - 1
      ...
      pktsum += len(pkt)
      time.sleep(0.1)
      sendp(pkt, iface=dst_if[j], verbose=False)
